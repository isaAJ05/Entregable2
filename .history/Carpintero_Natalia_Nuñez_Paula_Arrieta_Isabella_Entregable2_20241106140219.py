...
def portafolio_maximo_beneficio():
    ...
    print("acciones minimas: " + acciones_minimas, "correlacion maxima:"correlacion_maxima )
    acciones = list(G.nodes)
    portafolios_validos = []
    for i in range(acciones_minimas, len(acciones) + 1):
        for subset in itertools.combinations(acciones, i):
            subgraph = G.subgraph(subset)
            if all(abs(subgraph[u][v]['weight']) <= correlacion_maxima for u, v in subgraph.edges):
